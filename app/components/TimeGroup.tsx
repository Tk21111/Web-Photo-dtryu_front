...
export default function TimeGroup({ i , projs}: { i : string , projs: Proj[] | null }) {
  ...
  return (
        <div className="flex flex-col space-y-1.5 px-3 py-2 shadow-2xl rounded-3xl h-full justify-baseline content-center ">
            <div className="space-y-1 my-2">
                <div className="flex flex-row justify-between py-0.5 px-2.5">
                    ...
                    { !isOpen &&<ArrowDownFromLine onClick={() => setIsOpen(!isOpen)} className="bg-gray-800 rounded-2xl w-10 h-8 p-1.5"/>}
                    { isOpen && <ArrowUpFromLine onClick={() => setIsOpen(!isOpen)} className="bg-gray-800 rounded-2xl w-10 h-8 p-1.5" />}
            ...
            <div className="flex flex-col justify-center py-0.5 px-2.5">
                {isOpen && <ArrowUpFromLine onClick={() => setIsOpen(!isOpen)} className="bg-gray-800 rounded-2xl w-[100%] h-8 p-1.5" />}
                {!isOpen && <ArrowDownFromLine onClick={() => setIsOpen(!isOpen)} className="bg-gray-800 rounded-2xl w-[100%] h-8 p-1.5" />}
